...
class VulDataset(Dataset):
    def __init__(self, examples, tokenizer, window=384, stride=192,
                 tok_desc="[<DESC>]", tok_code="[<CODE>]", modality="both"):
        ...
        self.td = tok_desc
        self.tc = tok_code
        self.mod = modality  # both | code | desc
    ...
    def _build_seq(self, ex):
        if self.mod == "both":
            return f"{self.td} {ex.desc} {self.tc} {ex.code}"
        elif self.mod == "code":
            return f"{self.tc} {ex.code}"
        elif self.mod == "desc":
            return f"{self.td} {ex.desc}"
        else:
            raise ValueError("modality must be in {both, code, desc}")

    def __getitem__(self, idx):
        ex = self.examples[idx]
        seq = self._build_seq(ex)
...
def group_t5_blocks(model):
    return list(model.enc.encoder.encoder.block)
...
def main():
    ...
    ap.add_argument("--modality", type=str, default="both", choices=["both","code","desc"])
    args = ap.parse_args()
    ...
    train_ds = VulDataset(train_ex, tok, window=args.window, stride=args.stride, modality=args.modality)
    val_ds   = VulDataset(val_ex, tok, window=args.window, stride=args.stride, modality=args.modality)
    ...
    if os.path.exists(args.encoder_ckpt):
        ...
        try:
            ...
            model.enc.encoder.load_state_dict(sd, strict=False)
    ...
    for e in range(1, args.phase1_epochs + 1):
        model.train()
        for bt in tqdm(train_dl, desc=f"SVC P1 ({args.modality}) Ep{e}"):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, f"svc_best_{args.modality}.pt"))
        else:
            bad += 1
            if bad >= args.patience:
                break

    if os.path.exists(os.path.join(args.output_dir, f"svc_best_{args.modality}.pt")):
        model.load_state_dict(torch.load(os.path.join(args.output_dir, f"svc_best_{args.modality}.pt"), map_location=device))
    ...
    for b_idx, block in enumerate(blocks[:args.max_unfreeze_blocks]):
        ...
        for ep in range(1, args.epochs_per_block + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC P2 ({args.modality}) B{b_idx+1} Ep{ep}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
    ...
    if args.enable_phase3:
        ...
        for ep in range(1, args.phase3_epochs + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC P3 ({args.modality}) Ep{ep}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
