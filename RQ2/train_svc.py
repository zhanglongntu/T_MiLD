...
def main():
    ...
    ap.add_argument("--output_dir", type=str, default="./outputs_rq2")
    ...
    for e in range(1, args.phase1_epochs + 1):
        model.train()
        for bt in tqdm(train_dl, desc=f"SVC w/o DLD P1 Ep{e}"):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, "svc_wo_dld_best.pt"))
        else:
            bad += 1
            if bad >= args.patience:
                break

    if os.path.exists(os.path.join(args.output_dir, "svc_wo_dld_best.pt")):
        model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_wo_dld_best.pt"), map_location=device))
    ...
    for b_idx, block in enumerate(blocks[:args.max_unfreeze_blocks]):
        ...
        for ep in range(1, args.epochs_per_block + 1):
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC w/o DLD P2 Block{b_idx+1} Ep{ep}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
    ...
    if args.enable_phase3:
        ...
        for ep in range(1, args.phase3_epochs + 1):
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC w/o DLD P3 Ep{ep}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
