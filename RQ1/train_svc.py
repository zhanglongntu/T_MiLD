...
def main():
    ...
    for e in range(1, args.phase1_epochs + 1):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, "svc_best.pt"))
        else:
            bad += 1
            if bad >= args.patience:
                break

    if os.path.exists(os.path.join(args.output_dir, "svc_best.pt")):
        model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_best.pt"), map_location=device))
