...
def main():
    ...
    ap.add_argument("--output_dir", type=str, default="./outputs_no_tl")
    ...
    for e in range(1, args.phase1_epochs + 1):
        model.train()
        for bt in tqdm(train_dl, desc=f"SVC(noTL) P1 Epoch {e}"):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, "svc_no_tl_p1.pt"))
        else:
            bad += 1
            if bad >= args.patience:
                break

    if os.path.exists(os.path.join(args.output_dir, "svc_no_tl_p1.pt")):
        model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_no_tl_p1.pt"), map_location=device))
    ...
    for b_idx, block in enumerate(blocks[:args.max_unfreeze_blocks]):
        ...
        for e in range(1, args.epochs_per_block + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC(noTL) P2 Block{b_idx+1} Ep{e}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
            ...
            if metrics["f1"] > best_f1:
                best_f1, bad = metrics["f1"], 0
                torch.save(model.state_dict(), os.path.join(args.output_dir, "svc_no_tl_best.pt"))
            else:
                bad += 1
                if bad >= args.patience:
                    break
    ...
    if args.enable_phase3:
        if os.path.exists(os.path.join(args.output_dir, "svc_no_tl_best.pt")):
            model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_no_tl_best.pt"), map_location=device))
        for p in model.enc.parameters():
            p.requires_grad = True
        ...
        for e in range(1, args.phase3_epochs + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC(noTL) P3 Epoch {e}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
