...
def unfreeze_top_n_layers(model, n):
    layers = list(model.enc.encoder.encoder.layer)
    for p in model.enc.parameters(): p.requires_grad = False
    if n <= 0: return
    for lyr in layers[-n:]:
        for p in lyr.parameters(): p.requires_grad = True
...
def main():
    ...
    freeze_all(model)
    unfreeze_top_n_layers(model, args.sva_unfreeze_top)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.enc.state_dict(), os.path.join(args.output_dir, "sva_encoder_gcbert.pt"))
        else:
            bad += 1
