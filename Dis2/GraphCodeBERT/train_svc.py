...
def read_examples(excel_path):
    ...
    for i, r in df.iterrows():
        ...
        exs.append(Example(i, desc, code, cwe))
    return exs
...
class VulDataset(Dataset):
    ...
    def _windows(self, ids):
        if len(ids) <= self.W: return [ids]
        segs, i = [], 0
        while True:
            segs.append(ids[i:i+self.W])
            if i + self.W >= len(ids): break
            i += self.S
        return segs
    def __getitem__(self, idx):
        ...
        segs = self._windows(ids)
        return {"segments": segs, "label": ex.cwe_label, "sid": ex.idx}
...
def collate_fn(batch, pad_id):
    ...
    for i, b in enumerate(batch):
        ...
        labels.append(b["label"]); sids.append(b["sid"])
    ...
    if segs:
        ...
        for s in segs:
            ...
            input_ids.append(s)
        attn = [[1 if tok != pad_id else 0 for tok in s] for s in input_ids]
    ...
    return {"input_ids": input_ids, "attention_mask": attn, "owners": owners, "labels": labels, "sids": sids}
...
class GCBE_Encoder(torch.nn.Module):
...
class SVC_System(torch.nn.Module):
    def __init__(self, name, num_classes):
        super().__init__()
        self.enc = GCBE_Encoder(name)
        h = self.enc.encoder.config.hidden_size
        self.agg = SegmentAggregator(h)
        self.head = SVCHead(h, num_classes)
    def seg_embed(self, input_ids, attn):
        if input_ids.numel()==0:
            h = self.enc.encoder.config.hidden_size
            return torch.empty(0, h, device=input_ids.device)
    ...
    def forward(self, input_ids, attention_mask, owners):
        seg_embs = self.seg_embed(input_ids, attention_mask)
...
def group_roberta_layers(model):
    return list(model.enc.encoder.encoder.layer)
...
def evaluate(model, dl, device):
    ...
    for b in tqdm(dl, desc="Eval"):
        ...
        p_all.extend(pred.cpu().numpy().tolist())
    acc = accuracy_score(y_all, p_all)
    p, r, f1, _ = precision_recall_fscore_support(y_all, p_all, average="macro", zero_division=0)
    mcc = matthews_corrcoef(y_all, p_all)
    return {"acc": acc, "precision": p, "recall": r, "f1": f1, "mcc": mcc}

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--model_name_or_path", type=str, default="microsoft/graphcodebert-base")
    ap.add_argument("--train_file", type=str, required=True)
    ap.add_argument("--val_file", type=str, required=True)
    ap.add_argument("--output_dir", type=str, default="./outputs_gcbert")
    ap.add_argument("--encoder_ckpt", type=str, default="./outputs_gcbert/sva_encoder_gcbert.pt")
    ...
    train_dl = DataLoader(train_ds, batch_size=args.train_bs, shuffle=True, collate_fn=collate)
    val_dl   = DataLoader(val_ds, batch_size=args.eval_bs, shuffle=False, collate_fn=collate)
    ...
    for e in range(1, args.phase1_epochs + 1):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, "svc_best_gcbert.pt"))
        else:
            bad += 1
            if bad >= args.patience: break

    if os.path.exists(os.path.join(args.output_dir, "svc_best_gcbert.pt")):
        model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_best_gcbert.pt"), map_location=device))

    layers = list(reversed(group_roberta_layers(model)))
    best_f1, bad = 0.0, 0
    total_epochs = 0

    for b_idx, lyr in enumerate(layers[:args.max_unfreeze_blocks]):
        set_trainable(lyr, True)
        params = [
            {"params": lyr.parameters(), "lr": args.top_lr},
            {"params": model.head.parameters(), "lr": args.head_top_lr}
        ]
        opt = AdamW(params)
        for e in range(1, args.epochs_per_block + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC P2 L{b_idx+1} Ep{e}"):
                ids = bt["input_ids"].to(device)
                attn = bt["attention_mask"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
    ...
    if args.enable_phase3:
        if os.path.exists(os.path.join(args.output_dir, "svc_best_gcbert.pt")):
            model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_best_gcbert.pt"), map_location=device))
        set_trainable(model.enc, True)
        opt = AdamW(model.parameters(), lr=args.full_lr)
        best_f1, bad = 0.0, 0
        for e in range(1, args.phase3_epochs + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC P3 Epoch {e}"):
                ids = bt["input_ids"].to(device)
                attn = bt["attention_mask"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
