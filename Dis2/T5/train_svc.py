...
def main():
    ...
    ap.add_argument("--encoder_ckpt", type=str, default="./outputs_t5/sva_encoder_t5.pt")
    ...
    for e in range(1, args.phase1_epochs + 1):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, "svc_best_t5.pt"))
        else:
            bad += 1
            if bad >= args.patience:
                break

    if os.path.exists(os.path.join(args.output_dir, "svc_best_t5.pt")):
        model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_best_t5.pt"), map_location=device))
    ...
    for b_idx, block in enumerate(blocks[:args.max_unfreeze_blocks]):
        ...
        for e in range(1, args.epochs_per_block + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC P2 Block{b_idx+1} Ep{e}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
