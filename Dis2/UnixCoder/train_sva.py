...
class SVA_System(torch.nn.Module):
    def __init__(self, name, num_classes):
        ...
        self.head = SVAHead(self.enc.hidden, num_classes)
...
def unfreeze_top_n_layers(model, n):
    layers = list(model.enc.model.encoder.layer)  # RoBERTa-style
    for p in model.enc.parameters():
        p.requires_grad = False
    if n <= 0: return
    for b in layers[-n:]:
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        model.train()
        loss_sum = 0.0
        for bt in tqdm(train_dl, desc=f"SVA-UniX Epoch {epoch}"):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.enc.state_dict(), os.path.join(args.output_dir, "sva_encoder_unix.pt"))
        else:
            bad += 1
