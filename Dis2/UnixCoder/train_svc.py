...
class VulDataset(Dataset):
    def __init__(self, examples, tokenizer, window=384, stride=192, t_desc="[<DESC>]", t_code="[<CODE>]"):
...
class UniXEncoder(torch.nn.Module):
...
class SVC_System(torch.nn.Module):
    def __init__(self, name, num_classes):
        super().__init__()
        self.enc = UniXEncoder(name)
        self.agg = SegmentAggregator(self.enc.hidden)
        self.head = SVCHead(self.enc.hidden, num_classes)

    def seg_embed(self, input_ids, pad_id):
        if input_ids.numel()==0:
            return torch.empty(0, self.enc.hidden, device=input_ids.device)
...
def main():
    ...
    ap.add_argument("--model_name_or_path", type=str, default="microsoft/unixcoder-base")
    ap.add_argument("--train_file", type=str, required=True)
    ap.add_argument("--val_file", type=str, required=True)
    ap.add_argument("--output_dir", type=str, default="./outputs_unix")
    ap.add_argument("--encoder_ckpt", type=str, default="./outputs_unix/sva_encoder_unix.pt")
    ...
    if os.path.exists(args.encoder_ckpt):
        model.enc.load_state_dict(torch.load(args.encoder_ckpt, map_location=device), strict=False)
    ...
    for e in range(1, args.phase1_epochs + 1):
        model.train()
        for bt in tqdm(train_dl, desc=f"SVC-UniX P1 Ep{e}"):
        ...
        if metrics["f1"] > best_f1:
            ...
            torch.save(model.state_dict(), os.path.join(args.output_dir, "svc_best_unix.pt"))
        else:
            bad += 1
            if bad >= args.patience:
                break

    if os.path.exists(os.path.join(args.output_dir, "svc_best_unix.pt")):
        model.load_state_dict(torch.load(os.path.join(args.output_dir, "svc_best_unix.pt"), map_location=device))

    layers = list(reversed(group_unix_layers(model)))
    best_f1, bad = 0.0, 0
    total_epochs = 0

    for b_idx, block in enumerate(layers[:args.max_unfreeze_blocks]):
        set_trainable(block, True)
        params = [
            {"params": block.parameters(), "lr": args.top_lr},
        ...
        for e in range(1, args.epochs_per_block + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC-UniX P2 B{b_idx+1} Ep{e}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
    ...
    if args.enable_phase3:
        ...
        for e in range(1, args.phase3_epochs + 1):
            total_epochs += 1
            model.train()
            for bt in tqdm(train_dl, desc=f"SVC-UniX P3 Ep{e}"):
                ids = bt["input_ids"].to(device)
                owners = bt["owners"].to(device)
                y = bt["labels"].to(device)
                logits = model(ids, owners, pad_id)
